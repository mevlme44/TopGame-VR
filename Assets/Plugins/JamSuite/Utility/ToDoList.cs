...
namespace JamSuite
{
    ...
    public class ToDoInspector : Editor
    {
        ...
        protected virtual string ColorTag(string tag) {
            var hash = unchecked((uint)tag.ToLowerInvariant().GetHashCode());
            var hue = hash * 1f / uint.MaxValue;
            var color = Color.HSVToRGB(hue, 1f, 0.7f);
            return string.Concat("<color=#", ColorUtility.ToHtmlStringRGB(color), "><b>", tag, "</b></color>");
        }
        ...
        protected void DisplayItem(Rect r, int i, bool active, bool focus, List<string> source, List<string> dest) {
            ...
            if (Regex.IsMatch(text, @"^[A-Za-z\s,]+:")) {
                ...
                var tags = Array.ConvertAll(Regex.Split(text.Substring(0, colonIndex), @"[,\s]+"), ColorTag);
                displayText = string.Concat(string.Join(", ", tags), ":", text.Substring(colonIndex + 1));
            }
            ...
            else if (GUI.Button(leftRect, new GUIContent(displayText, text), styles.buttonLeft)) {
                plan.Add(() => {
